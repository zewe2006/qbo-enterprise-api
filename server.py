...
def _parse_grubhub_pdf(pdf_bytes: bytes) -> dict:
    ...
    # Search only the header (first ~4KB) and use a bounded pattern with no nested quantifiers
    # to avoid catastrophic backtracking if the (#\d+) marker isn't found.
    store_name = ""
    header = full_text[:4000]
    m = _re.search(r'Account[ \t]*\n([^\n]+(?:\n[^\n]+){0,3}?)\s*\(#\d+\)', header)
    if m:
